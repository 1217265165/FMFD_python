...
FREQ_FEATURE_PARAMS = {
    'X4': (0.0, 0.03),      # 频率标度非线性度 (std)，正常~0.01
    'X8': (0.0, 0.02),      # 本振泄漏，正常~0.009
    'X9': (0.0, 0.001),     # 调谐线性度残差，正常~0.0001
    'X14': (-0.2, 0.2),     # 低频段残差均值，正常~0.02
    'X15': (0.0, 0.05),     # 高频段残差标准差，正常~0.009
    'X16': (-0.01, 0.01),   # 互相关滞后/频移，正常~0
    'X17': (-0.1, 0.1),     # 频轴缩放因子，正常~0
    'X18': (-0.1, 0.1),     # 频轴平移因子，正常~0
}

# 特征权重 - 增加X16/X17/X18的权重（关键频率异常指标）
